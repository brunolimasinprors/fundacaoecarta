...
/*
 * Adiciona as metatags para indexação de buscadores
 * @return html
 * 
 * @author Bruno Mendes lima
 * @version 01/03/2016
 */
add_action('wp_head', 'adicionaMetatagsPaginas');
...
function adicionaMetatagsHome() {
    $args = array(
        'post_type' => 'area0_post_type',
        'post_status' => 'publish',
        'order' => 'DESC'
    );
    $query = new WP_Query($args);
    ...
    if ($query->have_posts()) {
        ...
        $objImagem = get_field('logo_area_0', $postId);
        if ($objImagem) {
        ...
        //> Resumo								
        $resumo = get_field('resumo_area_0', $postId);
        ...
        //> Palavras Chaves   
        $palavras_chaves_post = get_field('palavras_area_0', $postId);
